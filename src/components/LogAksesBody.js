...
const LogAksesBody = () => {
  ...
  useEffect(() => {
    const fetchInitialLogData = async () => {
      try {
        ...
        console.error("Error fetching initial log data:", error);
        setLoading(false);
      }
    };

    const fetchFullLogData = async () => {
      try {
        const response = await axios.get(
          process.env.REACT_APP_API_URL + "/activitylog",
          {
        ...
        console.error("Error fetching full log data:", error);
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, filterStatus, selectedMonth]);
  ...
  const exportToPdf = () => {
    ...
    const titlePositionX = 600 / 2;
    const titlePositionY = 70;
    doc.setFont("Cambria");
    ...
    const logoWidth = 115;
    const logoHeight = 33;
    const logoPositionX = doc.internal.pageSize.getWidth() - logoWidth - 37;
    ...
    const tableStartY = logoPositionY + logoHeight + 30;
                ...
                <tbody>
                  {loading ? (
                    ...
                    currentRows.map((entry) => (
                      <tr key={entry.id}>
                        <td>{entry.petugas_name}</td>
                        <td>{entry.petugas_nip}</td>
              ...
              <nav>
                <ul className="pagination">
                  ...
                  {Array.from({ length: 3 }, (_, index) => {
                    let pageIndex = currentPage - 1 + index;
                    ...
                    if (
                      pageIndex > 0 &&
                      pageIndex <= Math.ceil(filteredRows.length / rowsPerPage)
                    ) {
                      return (
                        <li
                          key={pageIndex}
